...
from ..api.models import LLMModel, ChatMessage, ChatResponse
...
class LLMManager:
    ...
    def __init__(self):
        self.openrouter_client: Optional[httpx.AsyncClient] = None
    ...
    def classify_request(self, messages: List[ChatMessage], context: Optional[Dict[str, Any]] = None) -> str:
        ...
        if any(keyword in last_message for keyword in ["code", "function", "class", "debug", "implement"]):
            return "code"
        elif any(keyword in last_message for keyword in ["plan", "design", "architecture", "strategy"]):
            return "planning"
        elif any(keyword in last_message for keyword in ["document", "explain", "describe", "write"]):
            return "documentation"
        elif any(keyword in last_message for keyword in ["review", "check", "analyze", "security"]):
            return "review"
        else:
            return "chat"
    
    def select_optimal_model(self, task_type: str, prefer_local: bool = False) -> Optional[str]:
        ...
        if prefer_local:
            local_models = [m.id for m in self.available_models if m.provider == "ollama" and m.is_available]
            for model_id in preferred_models:
                if any(model_id in local_id for local_id in local_models):
                    return next(local_id for local_id in local_models if model_id in local_id)
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        if model_info.provider == "openrouter":
            return await self._openrouter_chat_completion(messages, model, stream, **kwargs)
