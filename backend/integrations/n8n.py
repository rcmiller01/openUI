...
class N8NManager:
    ...
    async def initialize(self, api_key: Optional[str] = None):
        ...
        self.client = httpx.AsyncClient(
            base_url=f"{self.n8n_url}/api/v1",
            headers=headers,
            timeout=30.0
        )
            ...
            if response.status_code == 200:
                ...
                for workflow_data in workflows_data.get("data", []):
                    workflow = N8NWorkflow(
                        ...
                        status=WorkflowStatus(workflow_data.get("active", False) and "active" or "inactive"),
    ...
    async def create_workflow(self, workflow_definition: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Created n8n workflow: {workflow_data['name']} ({workflow_id})")
                return workflow_id
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                execution = WorkflowExecution(
                    ...
                    status=ExecutionStatus(execution_data.get("finished", False) and "success" or "running"),
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if execution_id in self.executions:
                    ...
                    execution.status = ExecutionStatus(execution_data.get("finished", False) and "success" or "running")
                    if execution_data.get("stoppedAt"):
                        execution.end_time = datetime.fromisoformat(execution_data["stoppedAt"].replace("Z", "+00:00"))
                    return execution
                ...
                execution = WorkflowExecution(
                    ...
                    end_time=execution_data.get("stoppedAt") and datetime.fromisoformat(execution_data["stoppedAt"].replace("Z", "+00:00")) or None
                )
                self.executions[execution_id] = execution
                return execution
            return None
    ...
    async def wait_for_execution(self, execution_id: str, timeout: float = 300.0) -> bool:
    ...
    async def setup_git_integration_workflow(self, repository_path: str) -> Optional[str]:
    ...
    async def trigger_git_commit_workflow(self, repository_path: str, commit_message: str, files: List[str]) -> Optional[str]:
        ...
        if git_workflow_id:
            return await self.execute_workflow(git_workflow_id, {
    ...
    async def trigger_code_review_workflow(self, repository_path: str, branch: str, pull_request_url: str) -> Optional[str]:
