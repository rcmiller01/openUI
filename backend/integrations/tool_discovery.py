...
class ToolDiscoveryManager:
        ...
        if "debug" in self.providers:
            ...
            if debug_manager.is_initialized:
                self.tools["debug_core"] = Tool(
                    ...
                    capabilities=["start_session", "set_breakpoint", "step_over", "evaluate"]
                )
    ...
    async def invoke_tool(self, tool_id: str, capability: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _invoke_lsp_tool(self, tool: Tool, capability: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Invoke LSP tool"""
        lsp_manager = self.providers["lsp"]
    ...
    async def _invoke_mcp_tool(self, tool: Tool, capability: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Invoke MCP tool"""
        mcp_manager = self.providers["mcp"]
        return await mcp_manager.invoke_tool(tool.source, tool.name.split("_", 1)[1], params)
    
    async def _invoke_n8n_tool(self, tool: Tool, capability: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Invoke n8n workflow"""
        n8n_manager = self.providers["n8n"]
        execution_id = await n8n_manager.execute_workflow(tool.source, params.get("data", {}))
        return {"execution_id": execution_id} if execution_id else {"error": "Failed to start workflow"}
    
    async def _invoke_debug_tool(self, tool: Tool, capability: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Invoke debug tool"""
        debug_manager = self.providers["debug"]
        ...
        if capability == "start_session":
            ...
            return {"session_id": session_id} if session_id else {"error": "Failed to start session"}
        elif capability == "set_breakpoint":
            bp_id = await debug_manager.set_breakpoint(
                params["session_id"], params["file_path"], params["line"], params.get("condition")
            )
            return {"breakpoint_id": bp_id} if bp_id else {"error": "Failed to set breakpoint"}
        # Add other debug capabilities...
        else:
            return {"error": f"Unknown debug capability: {capability}"}
    
    async def _invoke_native_tool(self, tool: Tool, capability: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Invoke native tool"""
    ...
    def get_usage_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_category": {
                tool_type.value: len([t for t in self.tools.values() if t.type == tool_type])
