...
async def test_models_prefer_ollama(monkeypatch):
    server = reload_server_with_env({'OPENROUTER_API_KEY': None, 'OLLAMA_BASE_URL': 'http://ollama.local'})
    ...
    monkeypatch.setattr(server, 'httpx_client', SimpleNamespace(get=fake_get, post=async_noop))
...
async def test_models_include_openrouter_when_key_set(monkeypatch):
    server = reload_server_with_env({'OPENROUTER_API_KEY': 'key123', 'OLLAMA_BASE_URL': 'http://ollama.local'})

    async def fake_get(url, *args, **kwargs):
        if 'openrouter.ai' in url:
            return FakeResp(200, {'data': [{'id': 'open:1', 'context_length': 4096, 'name': 'or'}]})
        return FakeResp(404, {})
...
async def test_chat_routes_to_ollama(monkeypatch):
    ...
    server.mock_models = [SimpleNamespace(id='ollama-1', name='ollama-1', provider='ollama', context_length=4096, is_available=True)]
    ...
    monkeypatch.setattr(server, 'httpx_client', SimpleNamespace(get=async_noop, post=fake_post))
...
async def test_chat_routes_to_openrouter(monkeypatch):
    ...
    server.mock_models = [SimpleNamespace(id='open:1', name='open:1', provider='openrouter', context_length=4096, is_available=True)]

    async def fake_post(url, *args, **kwargs):
        if 'openrouter.ai' in url:
            return FakeResp(200, {'choices': [{'message': {'content': 'hello from openrouter'}, 'finish_reason': 'stop'}], 'usage': {'total_tokens': 5}})
        raise RuntimeError('unexpected post')
...
async def test_chat_fallback_when_providers_fail(monkeypatch):
    ...
    server.mock_models = []
    monkeypatch.setattr(server, 'httpx_client', SimpleNamespace(get=async_noop, post=async_noop))
