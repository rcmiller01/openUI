...
coordinator: Optional[EnhancedAgentCoordinator] = None
tool_discovery: Optional[ToolDiscoveryManager] = None
...
async def health_check():
    ...
    return {
        ...
        "services": {
            ...
            "tool_discovery": tool_discovery is not None and tool_discovery.is_initialized,
        },
        "capabilities": {
            "enhanced_lsp": True,
...
async def get_code_completion(request: dict):
...
async def get_hover_info(request: dict):
...
async def execute_n8n_workflow(request: dict):
...
async def trigger_git_commit_workflow(request: dict):
    ...
    try:
        ...
        return {"execution_id": execution_id} if execution_id else {"error": "Failed to trigger workflow"}
    except Exception as e:
...
async def start_debug_session(request: dict):
    ...
    try:
        ...
        return {"session_id": session_id} if session_id else {"error": "Failed to start debug session"}
    except Exception as e:
...
async def set_breakpoint(request: dict):
    ...
    try:
        ...
        return {"breakpoint_id": breakpoint_id} if breakpoint_id else {"error": "Failed to set breakpoint"}
    except Exception as e:
...
async def git_commit(request: dict):
    ...
    try:
        if use_n8n and n8n_manager:
            ...
            result = await mcp_manager.git_commit(repository_path, commit_message, files)
            return {
                "method": "mcp",
                "result": result,
...
async def test_integrations():
    ...
    if mcp_manager and mcp_manager.is_initialized:
        ...
        results["mcp"] = {
            ...
            "connected_servers": len([s for s in server_status if s["state"] == "connected"])
        }
    else:
