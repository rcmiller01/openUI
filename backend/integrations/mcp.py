...
class MCPManager:
                ...
                if config["type"] == MCPServerType.STDIO and config.get("command"):
                    ...
                    try:
                        async with httpx.AsyncClient() as client:
                            response = await client.get(config["endpoint"] + "/health", timeout=2.0)
    ...
    async def _send_stdio_request(self, server: MCPServer, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _send_http_request(self, server: MCPServer, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def git_status(self, repository_path: str) -> Dict[str, Any]:
        ...
        return await self.invoke_tool("git_mcp", "git_status", {"path": repository_path})
    
    async def git_commit(self, repository_path: str, message: str, files: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    async def git_push(self, repository_path: str, remote: str = "origin", branch: str = "main") -> Dict[str, Any]:
        """Push changes using MCP git server"""
        return await self.invoke_tool("git_mcp", "git_push", {
            "path": repository_path,
            "remote": remote,
            "branch": branch
        })
    
    async def git_pull(self, repository_path: str, remote: str = "origin", branch: str = "main") -> Dict[str, Any]:
        """Pull changes using MCP git server"""
        return await self.invoke_tool("git_mcp", "git_pull", {
            "path": repository_path,
            "remote": remote,
            "branch": branch
        })